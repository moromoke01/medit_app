...
const API_URL = "https://healthconsultbackend.onrender.com/api/consultation";
...
export const fetchConsultingRequests = createAsyncThunk("consultation/fetchConsultingRequests", async (_, { getState, rejectWithValue}) => {
    try {
        const state = getState();
...
//fetch consulting Requests for the logged-in specialist alone
export const fetchConsultingRequestForSpecialist = createAsyncThunk("consultation/fetchConsultingRequestForSpecialist", async (_, { getState, rejectWithValue }) => {
    try {
        const state = getState();
        const token = state.auth.token;
        if (!token) {
        ...
        const response = await axios.get(`${API_URL}/getConsultationDataForSpecialist`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
//update a consulting request status
export const updateConsultingRequestStatus = createAsyncThunk("consultation/updateConsultingRequestStatus", async ({ id, status, message, subject }, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.token
        const response = await axios.patch(`${API_URL}/updateConsultationStatus/${id}`,
...
const consultingSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            // fetch consulting request for specialist
            .addCase(fetchConsultingRequestForSpecialist.pending, (state) => {
                state.isLoading = true;
                state.error = null;
            })
            .addCase(fetchConsultingRequestForSpecialist.fulfilled, (state, action) => {
                state.isLoading = false;
                state.consultingRequests = action.payload;
            })
            .addCase(fetchConsultingRequestForSpecialist.rejected, (state, action) => {
                state.isLoading = false;
                state.error = action.payload.error;
            })
            ...
            .addCase(updateConsultingRequest.pending, (state) => {
                state.isLoading = true;
                state.error = null;
            })
            .addCase(updateConsultingRequest.fulfilled, (state, action) => {
                state.isLoading = false;
                const index = state.consultingRequests.findIndex(
                    (request) => request._id === action.payload._id
                );
                if (index !== -1) {
                    state.consultingRequests[index] = action.payload;
                }
            })
            .addCase(updateConsultingRequest.rejected, (state, action) => {
                state.isLoading = false;
                state.error = action.payload.error;
            })
            ...
            .addCase(updateConsultingRequestStatus.pending, (state) => {
                state.isLoading = true;
                state.error = null;
            })
            .addCase(updateConsultingRequestStatus.fulfilled, (state, action) => {
                state.isLoading = false;
                const index = state.consultingRequests.findIndex(
                    (request) => request._id === action.payload._id
                );
            ...
            .addCase(updateConsultingRequestStatus.rejected, (state, action) => {
                state.isLoading = false;
                state.error = action.payload.error;
            })
